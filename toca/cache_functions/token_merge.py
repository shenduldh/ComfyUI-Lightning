...
def token_merge(cache_dic, tokens, current, fresh_indices, stale_indices):
    ...
    if current["layer"] % 1 == 0:
        fresh_tokens = torch.gather(
            input=tokens,
            dim=1,
            index=fresh_indices.unsqueeze(-1).expand(-1, -1, tokens.shape[-1]),
        )
        stale_tokens = torch.gather(
            input=tokens,
            dim=1,
            index=stale_indices.unsqueeze(-1).expand(-1, -1, tokens.shape[-1]),
        )
        method = "similarity"
        if method == "distance":
            descending = False
            ...
            stale_fresh_dist, stale_fresh_indices_allstale = torch.max(
                similarity, dim=2
            )

        saved_topk_stale = int((stale_fresh_dist > 0.995).sum(dim=1).min())
        merged_stale_sequence = torch.sort(
            stale_fresh_dist, dim=1, descending=descending
        )[1][:, :saved_topk_stale]
        stale_fresh_indices = stale_fresh_indices_allstale.gather(
            1, merged_stale_sequence
        )
