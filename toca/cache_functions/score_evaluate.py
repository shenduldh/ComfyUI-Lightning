...
def score_evaluate(cache_dic, tokens, current) -> torch.Tensor:
    ...
    if cache_dic["cache_type"] == "random":
        ...
        score2 = cache_dic["attn_map"][-1][current["layer"]].sum(
            dim=1
        )  # .mean(dim=0) # (B, N)
        # normalize
    ...
    if True and (cache_dic["force_fresh"] == "global"):
        soft_step_score = cache_dic["cache_index"][-1][current["layer"]][
            current["module"]
        ].float() / (cache_dic["fresh_threshold"])
        # soft_layer_score = cache_dic['cache_index']['layer_index'][current['module']].float() / (27)
        score = (
            score + cache_dic["soft_fresh_weight"] * soft_step_score
        )  # + 0.1 *soft_layer_score
